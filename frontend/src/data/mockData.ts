...
export const mockNetworkGraph: import('./schemas').NetworkGraphData = {
  nodes: [
    { id: 'A_1001', label: 'Ravi Kumar',      group: 'accused' },
    { id: 'A_1002', label: 'Suresh Patil',    group: 'accused' },
    { id: 'A_1003', label: 'Mahesh Gowda',    group: 'accused' },
    { id: 'C_2001', label: 'FIR-2024-0451',   group: 'case' },
    { id: 'C_2002', label: 'FIR-2024-0738',   group: 'case' },
    { id: 'C_2003', label: 'FIR-2025-0102',   group: 'case' },
    { id: 'V_3001', label: 'Anand Rao',       group: 'victim' },
    { id: 'V_3002', label: 'Lakshmi Devi',    group: 'victim' },
  ],
  links: [
    { source: 'A_1001', target: 'C_2001', label: 'Accused In' },
    { source: 'A_1001', target: 'C_2002', label: 'Accused In' },
    { source: 'A_1002', target: 'C_2002', label: 'Accused In' },
    { source: 'A_1002', target: 'C_2003', label: 'Accused In' },
    { source: 'A_1003', target: 'C_2001', label: 'Accused In' },
    { source: 'A_1003', target: 'C_2003', label: 'Accused In' },
    { source: 'V_3001', target: 'C_2001', label: 'Victim In' },
    { source: 'V_3001', target: 'C_2002', label: 'Victim In' },
    { source: 'V_3002', target: 'C_2002', label: 'Victim In' },
    { source: 'V_3002', target: 'C_2003', label: 'Victim In' },
  ],
};

// ── AI predictions mock data ──
function generatePredictions(): import('./schemas').PredictionPoint[] {
  const today = new Date();
  const districts = ['Bengaluru Urban', 'Mysuru', 'Hubli-Dharwad', 'Kalaburagi', 'Mangaluru'];
  const points: import('./schemas').PredictionPoint[] = [];

  for (const district of districts) {
    // Base risk varies by district
    const baseRisk = 30 + Math.floor(Math.random() * 40);
    for (let day = 0; day < 14; day++) {
      const date = subDays(today, 13 - day);
      const noise = Math.floor(Math.random() * 15) - 7;
      const trend = day > 9 ? Math.floor(Math.random() * 12) : 0; // uptick in last few days
      points.push({
        district,
        date: format(date, 'yyyy-MM-dd'),
        predictedRisk: Math.min(100, Math.max(0, baseRisk + noise + trend)),
        historicalAverage: baseRisk,
      });
    }
  }
