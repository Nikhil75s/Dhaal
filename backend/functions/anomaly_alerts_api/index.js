    ...
    try {
        ...
        for (const [districtIdStr, currentCount] of Object.entries(recentCounts)) {
            ...
            if (zScore > zScoreThreshold) {
                const spikePercentage = Math.round(((currentCount - historicalMeanForDistrict) / historicalMeanForDistrict) * 100);
                ...
                try {
                    await datastore.table('AnomalyAlerts').insertRow({
                        ...
                        Severity: "HIGH",
                        Message: message,
                        AlertTimestamp: new Date().toISOString().replace('T', ' ').substring(0, 19) // Safe Catalyst DateTime format
                    });
                } catch (insertErr) {
                    console.error("Failed to insert Anomaly into Data Store:", insertErr.message);
                }
            }
        }
